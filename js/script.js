...
function createMovieElements(arrFilms) {
    ...
    for (let i = 0; i < pages[currentIndexPages].length; i++) {
        ...
        movieRating.textContent = (arrFilms[indexRatingTitle].vote_average).toFixed(1);
...
async function getFilm() {
    try {
        const response = await api.get("/discover/movie?primary_release_date.gte=2014-09-15&primary_release_date.lte=2014-10-22")
...
async function searchFilm() {
    try {
        ...
        const response = await api.get(`/search/movie?${apiKey}language=pt-BR&include_adult=false&query=${query}`);
        const arrFilms = (response.data.results);
...
function createFeaturedMovie(arrFilms) {
    ...
    highlightLaunch.textContent = new Date(arrFilms.release_date).toLocaleDateString("pt-BR", {
        year: "numeric",
        month: "long",
        day: "numeric",
    });
...
!localStorage.getItem('currentThemeSrc') ? localStorage.setItem('./assets/light-mode.svg') : localStorage.getItem('currentThemeSrc');
